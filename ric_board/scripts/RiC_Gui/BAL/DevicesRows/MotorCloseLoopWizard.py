...
class MotorCloseLoopWizard(GUIWizard):
    @staticmethod
    ...
    def editWizard(self, data):
        ...
        encoderPort = Label(frame, text='Encoder port: ')
        self.select = StringVar(frame)
        self.select.set(data[1]['port'])
        portOp = apply(OptionMenu, (frame, self.select) + (tuple(data[0]) + tuple(data[1]['port'])))
        ...
        portOp.grid(row=14, column=1, sticky=E)
        typeOp.grid(row=15, column=1, sticky=E)
        ...
        option.grid(row=16, column=1, sticky=E)
        ...
        optionE.grid(row=17, column=1, sticky=E)
        ...
        add.grid(row=18, column=0, sticky=W)
    ...
    def createWizard(self, itemAvailable):
        ...
        portOp = apply(OptionMenu, (frame, self.select) + tuple(itemAvailable))
        portOp.grid(row=14, column=1, sticky=E)
