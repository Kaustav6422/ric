...
class CloseDiffPublishRepose(RiCHeader):
    ...
    def buildRequest(self, data):
        ...
        while self.index < ODOM_X:
            ...
            self.index += 1
        self._odomX = struct.unpack('<f', str(bytes))[0]
        bytes = bytearray()
        while self.index < ODOM_Y:
            bytes.append(data[self.index])
            self.index += 1
        self._odomY = struct.unpack('<f', str(bytes))[0]
        bytes = bytearray()
        while self.index < ODOM_THATE:
            bytes.append(data[self.index])
            self.index += 1
        self._odomTheta = struct.unpack('<f', str(bytes))[0]
        # bytes = bytearray()
        ...
        while self.index < TRAN_ROT_Z:
            bytes.append(data[self.index])
            self.index += 1
        self._translationRotationZ = struct.unpack('<f', str(bytes))[0]
        bytes = bytearray()
        while self.index < TRAN_ROT_W:
            bytes.append(data[self.index])
            self.index += 1
        self._translationRotationW = struct.unpack('<f', str(bytes))[0]
